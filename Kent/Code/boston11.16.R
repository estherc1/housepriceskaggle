...
mean((ridge.bestlambda - TotalY)^2)#MSE 0.06931806 
cv.ridgeoutprecict = predict(cv.ridge.out, s = bestlambda.ridge, newx = xtrain)
mean((cv.ridgeoutprecict - TotalY)^2)#MSE 0.0762545
...
mean((lasso.bestlambda - TotalY)^2)#MSE 0.2074333
cv.lassooutprecict = predict(cv.lasso.out, s = bestlambda.lasso, newx = xtrain)
mean((cv.lassooutprecict - TotalY)^2)#MSE 0.2053846
...
num_data=select_if(housestesttrain[1:1460,], is.numeric)

colnames(num_data)
cormatrix=cor(num_data,num_data$SalePrice)
allnumcor=cor(num_data)
library(Hmisc)
rcorrmatrix=rcorr(as.matrix(num_data))
rcorrmatrix$P
flattmatrix=flattenCorrMatrix(rcorrmatrix$r,rcorrmatrix$P)
flattmatrix=flattmatrix%>%arrange(desc(cor))

library(corrplot)
corrplot(allnumcor, type = "upper", order = "hclust", 
